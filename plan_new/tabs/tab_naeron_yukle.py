    ...
    with sekme3:
        ...
        if uploaded_file:
            try:
                ...
                """)
                st.success("✅ Veritabanı bağlantısı ve tablo hazır.")
    
                existing = pd.read_sql_query("SELECT * FROM naeron_ucuslar", conn)
                existing_keys = set((row["Uçuş Tarihi 2"], row["Öğrenci Pilot"], row["Görev"]) for _, row in existing.iterrows())
                st.write(f"🔍 Veritabanında zaten kayıtlı {len(existing_keys)} satır kontrol edildi.")
                ...
                for _, row in df.iterrows():
                    ...
                    if key not in existing_keys:
                        ...
                        st.info(f"⏭️ Atlandı (zaten vardı): {key}")
                ...
                if st.button("💾 Veritabanına Aktar"):
                    st.info("💽 Veritabanına kayıt işlemi başlatıldı...")
                    df_yeni.to_sql("naeron_ucuslar", conn, if_exists="append", index=False)
                    cursor_main = conn_main.cursor()
                    cursor_main.execute("REPLACE INTO naeron_log (tarih, kayit_sayisi) VALUES (?, ?)", (str(secilen_tarih), len(df_yeni)))
                    conn_main.commit()
                    conn.close()
                    st.success("🧾 Kayıtlar başarıyla veritabanına aktarıldı ve log güncellendi.")
